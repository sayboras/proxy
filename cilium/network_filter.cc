...
Network::FilterStatus Instance::onNewConnection() {
  ...
  callbacks_->addUpstreamCallback([this](
    ...
    port_policy_->useProxylib(l7proto_);

    // Initialize Go parser if requested
    if (config_->proxylib_.get() != nullptr && l7proto_ != "") {
    ...
    } else { // no Go parser, initialize logging
      log_entry_.InitFromConnection(policy_name, *option, stream_info, destination_identity, dst_address);
    }
